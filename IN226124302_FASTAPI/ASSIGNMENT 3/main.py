...
# _____________________- Ass 3 : end point : Discount _____________________________________
...
def bulk_discount(category:str=Query(...,description='Category to discount'),discount_percent:int=Query(...,ge=1,le=99,description='% off'),):
    ...
    'updated_count':len(updated),'updated_products':updated,}
...
# ________________________ Ass 3 end point : AUDIT ______________________________________
...
def product_audit():
    ...
    return {'total_products':len(products),'in_stock_count':len(in_stock_list),'out_of_stock_names':[p['name'] for p in out_stock_list],
    'total_stock_value':stock_value,'most_expensive':{'name':priciest['name'],'price':priciest['price']},}
